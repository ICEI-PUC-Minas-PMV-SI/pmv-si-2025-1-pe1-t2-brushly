...
// Evento para ativar corte (exemplo, seu botão cortar deve ativar essa variável)
document.querySelectorAll('.sidebar-icon').forEach(icon => {
  if (icon.innerText.includes('Cortar')) {
    icon.addEventListener('click', () => {
      ...
      alert("Selecione uma área com o mouse e clique em 'Confirmar Corte'.");
    });
  }
});

// Eventos de mouse para selecionar área de corte
...
canvas.addEventListener('mousemove', (e) => {
  ...
  drawImageWithAspectRatio(currentImage); // Redesenha a imagem
  ctx.strokeStyle = 'red';
  ctx.lineWidth = 2;
  ctx.strokeRect(
    cropStartX,
...
canvas.addEventListener('mouseup', () => {
  if (!isCropping) return;
  document.getElementById('crop-confirm-button').style.display = 'inline-block';
});

// Botão confirmar corte
document.getElementById('crop-confirm-button').addEventListener('click', () => {
  ...
  // Escalas para converter coordenadas do canvas para a imagem original
  ...
  // Cria um canvas temporário para recortar a imagem original
  const tempCanvas = document.createElement('canvas');
  const tempCtx = tempCanvas.getContext('2d');
  tempCanvas.width = sWidth;
  tempCanvas.height = sHeight;

  // Desenha a área recortada da imagem original no canvas temporário
  tempCtx.drawImage(currentImage, sx, sy, sWidth, sHeight, 0, 0, sWidth, sHeight);

  // Agora redimensiona o canvas principal para o tamanho do corte, e desenha essa imagem recortada
  canvas.width = width;
  canvas.height = height;
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.drawImage(tempCanvas, 0, 0, width, height);

  // Atualiza currentImage para o novo recorte, para poder continuar editando a imagem cortada
  ...
  isCropping = false;
  document.getElementById('crop-confirm-button').style.display = 'none';
